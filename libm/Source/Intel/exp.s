/****************************************************************************
	...
	Implementation of the C99 exp function for the MacOS X i386 and x86_64 ABIs.	
...
small_poly: .quad 0x3ff0000000000000
			.quad 0x3fdffffffffffe1f
			.quad 0x3fc55555555553f0
			.quad 0x3fa55555ca407ccb
...
abs_mask:	.quad 0x7fffffffffffffff	// Constants needed on x86_64
small_cut:	.quad 0x3f662e42fefa39ef	// because there are no 8-byte
large_cut:	.quad 0x011ff4e8de8082e3	// immediates
tiny_cut:	.quad 0xfd19d1bd0105c611
huge_cut:	.quad 0x01211d42457337d4
#endif

overflow:	.quad 0x40862e42fefa39ef	// Overflow boundary
underflow:	.quad 0xc086232bdd7abcd2	// Underflow boundary
...
ENTRY(exp)
	...
	call		0f
0:	pop			%ecx
	and			$0x7fffffff,		%eax	// hi word of |x|
	sar			$31,				%edx	// (x < 0) ? -1 : 0
	sub			$0x3f662e43,		%eax
	add			$64,				%edx	// bias = (x < 0) ? 63 : 64
	cmp			$0x011ff4e7,		%eax	// if  (log(2) |x| < 0x1p-8ish or log(2) |x| > 1022ish or isnan(x))
	ja			2f							//		goto 2
#endif
	
	// Code path for 0x1.0p-8 < log(2) |x| < 1022
1:
											// Value						Notes
	movapd		%xmm0,				%xmm1	// --------------------------------------------------------------
	...
	movd		AX_P,				%xmm4
	psllq		$52,				%xmm4	// n shifted into the exponent bits of a double
											
	movsd		8(DX_P, CX_P, 1),	%xmm0	// exp2(fhi)					accurate to 74 bits
	
	mulsd		%xmm0,				%xmm2
	addsd		%xmm2,				%xmm0	// exp2(fhi + flo)				accurate to .51625 ulp
	
	paddd		%xmm4,				%xmm0	// exp2(n + fhi + flo) = exp(x)	multiply by 2**n is done by adding n to
											//								the exponent of exp2(fhi + flo).
...
2:	jg			4f							// if (log(2) |x| > 1022 or isnan(x))
											//    goto 4
	
	// Code path for log(2) |x| < 0x1.0p-8
#if defined( __x86_64__ )
	cmp		REL_ADDR(tiny_cut),		%rax	// if (|x| < 0x1p-55)
#else
	cmp			$0xfd19d1bd,		%eax	// if (|x| < 0x1p-55ish)
#endif
	jbe			3f							//	  goto 3
	
	// Minimax polynomial approximation to exp( ) on [ -lg(e) 0x1.0p-8, lg(e) 0x1.0p-8 ]
	//
	// The approximation has the form 1.0 + x p(x), where p(x) has at most 1 ulp error.
	// Because |x| is < 0x1.0p-8, then, the error of the final result is bounded by .508125 ulp
...
4:
#if defined( __x86_64__ )
	cmp		REL_ADDR(huge_cut),		%rax	// if (the result will be NaN, inf, or zero)
#else
	cmp			$0x01211d43,		%eax	// if (the result will be NaN, inf, or zero)
#endif
	jg			5f							//	  goto 5

	// Here we handle cases very close to the over/underflow boundary.
	// Compare x with zero to determine whether we're in the overflow or underflow case
	xorpd		%xmm1,				%xmm1
	comisd		%xmm1,				%xmm0	// if (x > 0.0)
	jnc			6f							//	  goto 6
	
#if defined( __i386__ )
	// One extra test is needed in 32-bit mode to determine exactly which side of the underflow
	// boundary we are on.
	
	movsd	REL_ADDR(underflow),	%xmm1
	comisd		%xmm1,				%xmm0	// if (x >= underflow boundary)
	jnc			1b							//    go back into the mainline
											// otherwise, fall through into underflow code
#endif
	
	// (near) overflows have been correctly handled, we handle gradual underflows here
	...
	movsd	REL_ADDR(tiny_val),		%xmm6	// load 0x1p-1022
	mulsd		%xmm6,				%xmm6	// multiply to force underflow flag.
	
	movapd		%xmm1,				%xmm2
	mulsd	REL_ADDR(lge),			%xmm1
	cvtsi2sd	AX_P,				%xmm0
	...
	movapd		%xmm2,				%xmm4
	movsd	REL_ADDR(split_mask),	%xmm3
	mulsd	REL_ADDR(lge_lo),		%xmm4
	movsd	REL_ADDR(lge_hi),		%xmm5
	...
	add			$1022,				AX_P	// add 1022 to n before we place it into the exponent bits
	movd		AX_P,				%xmm4
	psllq		$52,				%xmm4
											
	movsd		8(DX_P, CX_P, 1),	%xmm1
	paddd		%xmm4,				%xmm1	// hi = exp2(1022 + n + fhi)
	
	mulsd		%xmm1,				%xmm2
	
	movapd		%xmm1,				%xmm3
	addsd		%xmm0,				%xmm1	// 1.0 + hi					rounded to 53 bits
	movapd		%xmm1,				%xmm4
	subsd		%xmm0,				%xmm1	// (1.0 + hi) - 1.0			exact
	subsd		%xmm1,				%xmm3	// hi - (1.0 + hi) - 1.0	rounding error from 1.0 + hi
	addsd		%xmm3,				%xmm2	// exp2(flo) + rounding error from above
	
	addsd		%xmm2,				%xmm4	// (1.0 + hi) + exp2(flo) + round error
											//							1.0 forces the round to happen
											//							in the correct place for the
											//							denormal value
											
	xorpd		%xmm4,				%xmm0	// set correct exponent for denormal
...
6:	// Here we need to do the floating-point compare to determine exactly which side
	// of the overflow boundary we are on.
	movsd	REL_ADDR(overflow),		%xmm1
	comisd		%xmm0,				%xmm1	// if (x <= overflow boundary)
	jnc			1b							//    go back into the mainline
											// otherwise, fall through into overflow code

7:	// Handle positive overflows with multiplication by huge val
	// Sets overflow, inexact for finite values, gives exact
	// infinity for +inf
	mulsd	REL_ADDR(huge_val),		%xmm0
...
8:	// Handle NaN by addition to quiet.
	addsd		%xmm0,				%xmm0
