 ...
 *
 * This file implements the C99 tanhf function for the MacOS X __i386__ and __x86_64__ abis.
 */
...
// |x| < 11, table-based reduction
	...
	lea		RELATIVE_ADDR(tanhf_table), DX_P
	...
	mulpd		%xmm2,			%xmm2		// xmm2 <-- { l2 , l2 }
		...
		movapd	(DX_P,AX_P,1),	%xmm6		// xmm6 <-- { sinh h, cosh h }
	...
	mulpd		%xmm0,			%xmm6		// xmm6 <-- { sinh h sinh l, cosh h cosh l }
	mulpd		%xmm0,			%xmm5		// xmm5 <-- { cosh h sinh l, sinh h cosh l }

#if defined(__SSE3__)
	haddpd		%xmm6,			%xmm6		// xmm6 <-- cosh |x|
	haddpd		%xmm5,			%xmm5		// xmm5 <-- sinh |x|
#endif
	
	divsd		%xmm6,			%xmm5		// xmm6 <-- sinh |x| / cosh |x|
	cvtsd2ss	%xmm5,			%xmm0
	orps		%xmm1,			%xmm0
#if defined(__i386__)
...
2: // |x| >= 11,
	...
	andnps		%xmm0,			%xmm1		// xmm1 <-- signbit(x)
	movl		$0x3f800000,	%eax
	movd		%eax,			%xmm0		// xmm0 <-- 1
	orps		%xmm1,			%xmm0		// xmm0 <-- copysign(1,x)
	
	je			3f							// Jump if |x| = infinity to avoid inexact
	
	movl		$0x01000000,	%edx
	movd		%edx,			%xmm2		// xmm2 <-- tiny
	orps		%xmm1,			%xmm2		// xmm2 <-- copysign(tiny,x)
	
	subss		%xmm2,			%xmm0		// xmm0 <-- copysign(1 - tiny,x), inexact is set
3:
#if defined(__i386__)
	movss		%xmm0,			FRAME_SIZE( STACKP )
	flds		FRAME_SIZE( STACKP )
