...
ios_base::Init::Init()
{
    if (__gnu_cxx::__exchange_and_add_dispatch(&_S_refcount, 0) == 0)
    {
        // If _S_refcount looks like 0, we need to lock a mutex
        pthread_mutex_lock(&get_mut());
        // If _S_refcount is still 0, then initialize
        if (_S_refcount == 0)
        {
            try
            {
        ...
        // Else _S_refcount != 0 which means initialization is complete, else
        //   I couldn't have gotten the mutex.
            ++_S_refcount;
        pthread_mutex_unlock(&get_mut());
    }
    else  // _S_refcount >= 2
